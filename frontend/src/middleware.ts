...
export function middleware(request: NextRequest) {
  ...
  if (payload && isPublicPath) {
    const dashboardPath =
      payload.role === 'DOCTOR' ? '/doctor/dashboard' : '/patient/dashboard';
  ...
  if (payload) {
    ...
    if (role === 'PATIENT' && pathname.startsWith(DOCTOR_PREFIX)) {
      return NextResponse.redirect(new URL('/patient/dashboard', request.url));
    }

    // DOCTOR trying to access patient routes
    if (role === 'DOCTOR' && pathname.startsWith(PATIENT_PREFIX)) {
